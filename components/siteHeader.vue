...
const { isOpened, isSticky, handleScroll, headerClasses, logoClasses, navClasses, linkClasses, burgerButtonClasses, isAuthed } = useHeaderData(isMain);
onMounted(() => {
        ...
        <div class="w-full py-4 px-5  md:px-20 flex items-center max-lg:flex-wrap"
            ...
            <div class="flex items-center max-lg:ml-auto lg:basis-full justify-end h-8">
                <appButton :label="isAuthed ? 'В профиль' : 'Войти'" :to="isAuthed ? '/profile' : '/login'"
                    variant="primaryOutline" :extra="`${isMain ? '!text-gray-700 dark:!text-white' : ''}`" />
