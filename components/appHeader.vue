...
<template>
    <div>
        <header class="bg-main dark:bg-darkMain dark:border-b-2 dark:border-b-thirdary">
            <nav class="container mx-auto py-2 flex flex-row justify-around  items-center flex-wrap">
                <div class="order-1 xl:basis-2/12 w-1/2"><img
                        :src="$colorMode.value == 'dark' ? '/logo2.png' : '/logo.png'"
                ...
                <div :class="{ 'max-xl:h-0 max-xl:opacity-0 max-xl:*:pointer-events-none': !isOpened, ' max-xl:py-2.5 max-xl:flex max-lg:h-[9.5rem] max-xl:h-[8.5rem] ': isOpened }"
                    ...
                    <NuxtLink
                        class="text-xl hover:text-gray-500 text-black dark:text-gray-300 dark:hover:text-white transition duration-300 outline-0 text-nowrap text-center"
                ...
                <div class="xl:order-3 order-2 xl:basis-3/12 w-1/2 flex justify-end gap-4 ">
                    <NuxtLink
                        class="text-xl rounded-3xl text-nowrap shadow-lg hover:shadow-gray-400 px-4 max-lg:hidden py-[0.6rem] flex items-center outline-0 bg-thirdary text-white dark:bg-[#415780] dark:hover:bg-thirdary dark:hover:shadow-[#415780] transition-all duration-500"
                        :to="isAuthed ? '/profile' : '/login'">{{ !isAuthed ? "Вход и регистрация" : "Перейти в профиль"
                        }}
                    </NuxtLink>
